...
This Terraform configuration creates the STCloudScan IAM role and policies to allow ST Cloud to scan your AWS account, with optional S3 integration for storing scan reports.
...
- `account_id` (optional): AWS Account ID that will assume the role (defaults to ST Cloud: "232136659152")
- `iam_principal` (optional): IAM principal pattern allowed to assume the role (defaults to ST Cloud: "role/*")
